...
public class CtrAngendamento implements ActionListener, InternalFrameListener, ListSelectionListener {
	...
	public void valueChanged(ListSelectionEvent e) {
		...
		// Na classe view do FrmAgendamento a variável cbPaciente deve ser a sua propriedade setEditable(true);
		this.frmAngedamento
			.getCbPaciente()
				.setSelectedItem(this.voAgendamento.getPaciente());
		
		this.frmAngedamento
			.getFtxtData()
				.setText(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
					.format(this.voAgendamento.getDataConsulta()));
	...
	private void gravar() {
		...
		VoPaciente voPaciente =
				(VoPaciente) this.frmAngedamento
					.getCbPaciente()
						.getSelectedItem();
		...
		try {
			...
			if(this.daoRegistroConsulta.consultar(idConsulta).size() > 0) {
				...
				JOptionPane.showMessageDialog(this.frmAngedamento
						, "Este agendamento já foi consolidado com uma consulta. Não é possível a exclusão."
						, "Aviso"
						, JOptionPane.WARNING_MESSAGE);
				return;
	...
	private void atualizarJTable() {
		...
		try {
			...
			if(this.frmAngedamento.getTxtFiltro().getText().length() == 0) {
				...
				this.voAgendamentos = daoAgendamento
						.consultar(this.frmAngedamento.getTxtFiltro().getText());
	...
	private void popularComboPaciente() {
		...
		try {
			...
			DefaultComboBoxModel<VoPaciente> cmbModelo =
					((DefaultComboBoxModel) this.frmAngedamento.getCbPaciente().getModel());
	...
	private void gerarRelatorioLista() {
		...
		try {
			...
			JRBeanCollectionDataSource jrbcds =
					new JRBeanCollectionDataSource(this.voAgendamentos);
			
			JasperPrint print =
					JasperFillManager.fillReport(
							"relatorios/RelAgendamento.jasper"
							, null
							, jrbcds);
