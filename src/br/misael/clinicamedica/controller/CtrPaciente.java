...
public class CtrPaciente extends Component
		implements ActionListener, KeyListener, InternalFrameListener, ListSelectionListener {
	...
	public CtrPaciente(FrmMenu frmMenu) {
		...
		this.frmPaciente.getTable().getSelectionModel().addListSelectionListener(this);
	...
	public void keyReleased(KeyEvent e) {
		...
		/*
		 * Limitando a digitação de carateres em campos de texto anexando aos mesmos, o
	...
	private void carregarFoto() {
		...
		try {
			...
			if (resultado == JFileChooser.CANCEL_OPTION || resultado == JFileChooser.ABORT) {
				return;
			}
			...
			// TODO remover esta regra daqui, regras devem ficar nas classes do pacote
			// Business
			if (imageLoader.getImageHeight() > h || imageLoader.getImageWidth() > w) {

				JOptionPane.showMessageDialog(this.frmPaciente,
						"A imagem não deve exceder as dimenssões de " + h + " x " + w + " pixels.", "Alerta",
			...
			JOptionPane.showMessageDialog(this.frmPaciente, "Falha ao carregar imagem. Causado por: " + e.getMessage(),
	...
	public void valueChanged(ListSelectionEvent e) {
		...
		this.frmPaciente.getFtxtDataNascimento()
				.setText(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(this.voPaciente.getDataNascimento()));
		...
		try {
			...
			JOptionPane.showMessageDialog(this.frmPaciente, "Falha ao carregar a foto. Causa: " + ex.getMessage(),
					"Erro", JOptionPane.ERROR_MESSAGE);
	...
	private void gravar() {
		...
		try {
			...
			dataNascimento = new Timestamp(
					formatoData.parse(this.frmPaciente.getFtxtDataNascimento().getText()).getTime());

		} catch (ParseException e) {

			JOptionPane.showMessageDialog(this.frmPaciente, "Data inválida.", "Erro", JOptionPane.ERROR_MESSAGE);
			return;

		}

		int respostaDialogo = JOptionPane.showConfirmDialog(this.frmPaciente, "Confirma gravação?", "",
		...
		// Campos requisitados.
		if (nome.length() == 0 || dataNascimento == null // TODO corrigir: a dataNascimento nunca é nula se não
															// informada
				|| endereco.length() == 0) {

			JOptionPane.showMessageDialog(this.frmPaciente, "Informe os campos requisitados.", "Aviso",
					JOptionPane.WARNING_MESSAGE);
			return;
		...
		try {
			...
			voPaciente.setTelefone(telefone.replace("(", "").replace(")", "").replace("-", "").replace(" ", ""));
			...
			JOptionPane.showMessageDialog(this.frmPaciente, resultado, "", JOptionPane.INFORMATION_MESSAGE);

		} catch (ClassNotFoundException e) {

			JOptionPane.showMessageDialog(this.frmPaciente, "Erro ao gravar.\n\nDetalhes: " + e.getMessage(), "Erro",
	...
	private void excluir() {
		...
		int respostaDialogo = JOptionPane.showConfirmDialog(this.frmPaciente, "Confirma exclusão?", "",
				JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
		...
		if (voPaciente.getIdPaciente() == 0) {
			...
			JOptionPane.showMessageDialog(this.frmPaciente, "Selecione um registro da tabela para excluir.", "",
					JOptionPane.WARNING_MESSAGE);
		...
		try {
			...
			if (agendamentos != 0) {
				...
				JOptionPane.showMessageDialog(this.frmPaciente,
						"O paciente " + voPaciente.getNome() + " possui " + agendamentos + " "
			...
			JOptionPane.showMessageDialog(this.frmPaciente, "Registro excluido.", "", JOptionPane.INFORMATION_MESSAGE);

		} catch (ClassNotFoundException e) {

			JOptionPane.showMessageDialog(this.frmPaciente, "Erro ao excluir.\n\nDetalhes: " + e.getMessage(), "Erro",
					JOptionPane.ERROR_MESSAGE);
	...
	private void atualizarJTable() {
		...
		try {
			...
			if (this.frmPaciente.getRdbtnAtivo().isSelected()) {
				...
				voPacientes.stream().filter(p -> p.getNome().contains(filtroNome) && p.isAtivo() == true).forEach(p -> {
					pacientesFiltrados.add(p);
				});

			} else if (this.frmPaciente.getRdbtnInativo().isSelected()) {

				voPacientes.stream().filter(p -> p.getNome().contains(filtroNome) && p.isAtivo() == false)
						.forEach(p -> {
							pacientesFiltrados.add(p);
						});
				...
				voPacientes.stream()
						.filter(p -> p.getNome().contains(filtroNome) && (p.isAtivo() == false || p.isAtivo() == true))
						.forEach(p -> {
							pacientesFiltrados.add(p);
						});
			...
			if (voPacientes != null) {
				...
				this.frmPaciente.getTable().setModel(new TblPaciente(voPacientes));

				this.frmPaciente.getTable().setDefaultRenderer(Object.class, new CelTblPaciente());

			}

		} catch (ClassNotFoundException e) {

			JOptionPane.showMessageDialog(this.frmPaciente, "Erro ao consultar.\n\nDetalhes: " + e.getMessage(), "Erro",
					JOptionPane.ERROR_MESSAGE);
	...
	private void gerarRelatorioLista() {
		...
		// Para utilizar um filtro para relatório ou aplicar os filtros antes, alimentar
		// a lista e passa-la para o JasperReport.
		...
		try {
			...
			if (this.voPacientes == null) {
				...
				JOptionPane.showMessageDialog(this.frmPaciente, "Execute uma consulta antes.", "Aviso",
						JOptionPane.WARNING_MESSAGE);
				return;
			...
			if (this.voPacientes.size() == 0) {
				...
				JOptionPane.showMessageDialog(this.frmPaciente, "Não há registros para exibir o relatório.", "Aviso",
						JOptionPane.WARNING_MESSAGE);
				return;
			...
			JOptionPane.showMessageDialog(this.frmPaciente, "Não foi possível abrir o relatório.\n\n" + ex.getMessage(),
					"Erro", JOptionPane.ERROR_MESSAGE);
			ex.printStackTrace();
